/**
 * ============================================================================
 * 🖤 PROPUESTA: ¿QUIERES SER MI NOVIA? (JUKARY & ANTONIO)
 * ============================================================================
 */
...
class EscenaIntro extends Phaser.Scene {
    ...
    prepararSobre(midX, midY) {
        ...
        this.textoMagico = this.add.text(0, 0, '', { fontSize: '24px', fill: '#ffffff', fontFamily: 'Eagle Lake', align: 'center', wordWrap: { width: 320 } }).setOrigin(0.5);
    ...
    crearBotonCristalMistico(x, y) {
        ...
        this.emisorBot = this.add.particles(0, 0, 'chispa', { speed: { min: 10, max: 40 }, scale: { start: 0.3, end: 0 }, alpha: { start: 0.8, end: 0 }, lifespan: 1200, blendMode: 'ADD', frequency: 60, tint: [0xd4af37, 0xffaa00, 0xffffff] });
    ...
    crearBotonEntrar() {
        ...
        const sello = this.add.circle(midX, midY, 50, 0x333333).setInteractive({ useHandCursor: true }).setDepth(200);
...
class EscenaJuego extends Phaser.Scene {
    ...
    configurarCajasYMonedas(cp) {
        ...
        const ms = ["¡HOLA JUKARY! ESTO ES SÓLO EL COMIENZO.", "CADA NIVEL ES UN MOMENTO ESPECIAL CONTIGO.", "ANTONIO TIENE UNA PREGUNTA PARA TI.", "¡SIGUE ADELANTE, FALTA MUY POCO!", "ERES LA MEJOR COMPAÑERA DE AVENTURAS.", "¡LO LOGRASTE! MIRA HACIA ADELANTE..."];
    ...
    finalizarJuego() {
        ...
        this.tweens.add({ targets: this.perro, x: this.pareja.x - 150, duration: 1500, onComplete: () => this.escribirMensajeFinal() });
    }
    escribirMensajeFinal() {
        let m = "¡LEVEL CLEAR!\n\nJukary, has llegado al final del camino...\n\nAntonio tiene algo que decirte:\n\n¿QUIERES SER MI NOVIA? 🖤";
        this.txtPerro = this.add.text(this.cameras.main.scrollX + (this.cameras.main.width / 2), this.cameras.main.height / 3, "", { fontSize: '24px', fill: '#ffd700', fontFamily: 'Cinzel Decorative', align: 'center', stroke: '#000', strokeThickness: 5, wordWrap: { width: 600 } }).setOrigin(0.5).setDepth(9999);
        let ci = 0;
        this.time.addEvent({ delay: 60, repeat: m.length - 1, callback: () => { this.txtPerro.text += m[ci]; ci++; if (ci === m.length) { this.mostrarBotonesRespuesta(); } } });
    }
    mostrarBotonesRespuesta() {
        ...
        let btnSi = this.add.rectangle(camX - 120, camY, 180, 60, 0x00ff00).setDepth(10000).setInteractive({useHandCursor:true});
        this.add.text(camX - 120, camY, '¡SÍ! 💍', { fontSize: '22px', fill: '#000', fontWeight: 'bold' }).setOrigin(0.5).setDepth(10001);
        let btnNo = this.add.rectangle(camX + 120, camY, 180, 60, 0xff0000).setDepth(10000).setInteractive({useHandCursor:true});
        let txtNo = this.add.text(camX + 120, camY, 'NO 😢', { fontSize: '22px', fill: '#000', fontWeight: 'bold' }).setOrigin(0.5).setDepth(10001);
        btnSi.on('pointerup', () => { window.open('https://wa.me/522297666724?text=¡SÍ,%20Antonio!%20Acepto%20ser%20tu%20novia%20🖤', '_blank'); });
        btnNo.on('pointerover', () => { btnNo.x = Phaser.Math.Between(camX - 200, camX + 200); btnNo.y = Phaser.Math.Between(camY - 100, camY + 50); txtNo.x = btnNo.x; txtNo.y = btnNo.y; });
    }
}

const config = { type: Phaser.AUTO, scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH, parent: 'game-container', width: 1024, height: 600 }, physics: { default: 'arcade', arcade: { gravity: { y: 1900 }, debug: false } }, scene: [EscenaIntro, EscenaSeleccion, EscenaJuego] };
const game = new Phaser.Game(config);
window.addEventListener('load', () => { setTimeout(() => { if (game.scale) { game.scale.refresh(); } }, 400); });
